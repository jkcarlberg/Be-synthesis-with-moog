...
c*****write out the opacities
     ...
     .      dlog10(sigH2(i))
         ...
         do i=1,ntau
            write (nf1out,1007) i, nint(t(i)), dlog10(kaplam(i)),
     .      dlog10(aC1(i)),  dlog10(aMg1(i)), dlog10(aMg2(i)), 
     .      dlog10(aAl1(i)), dlog10(aSi1(i)), dlog10(aSi2(i)), 
     .      dlog10(aFe1(i)), dlog10(aCH(i)), dlog10(aNH(i))
         enddo
      endif
...
c1003  format (/'log opacities due to "metal" edges at a wavelength',
c     .       ' of ',f10.2,' A'/' i  T(i)  kaplam     aC1    aMg1', 
c     .       '    aMg2    aAl1    aSi1    aSi2    aFe1')
1003  format (/'log opacities due to metals, CH, & NH at a wavelength',
     .       ' of ',f10.2,' A'/' i  T(i)  kaplam     aC1    aMg1', 
     .       '    aMg2    aAl1    aSi1    aSi2    aFe1    aCH     aNH')
...
1006  format ('      kaplam(i) = kaplam(i) * ',
     .        '((', f6.2, ' * 10000)/t(i))')
1007  format (i2,i6,12f8.2)
1008  format ('Reprint NH',e10.2,' at ntau 1')
